...
import { type FeedbackContainer } from "@type/container"
    ...
    <section className={`${hp.feedback}`}>
      ...
      >
        <div className={`${common.txtCenter}`}>
          <span className={`${common.tag}`}>{content.feedback.title}</span>
        </div>
        <div className={`${hp.feedbackItems} ${grid.grid1}`} content-step={step} style={{ left: -step * 100 + '%' }} {...swipeHandlers}>
          {feedback && feedback.map((feedback: FeedbackContainer, index: number) => {
            return (
